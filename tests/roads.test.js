...
describe('automatic road construction', () => {
  ...
  it('rejects roads shorter than two cells, longer than twenty cells, and road intersections', () => {
    const state = createGameState(); addTestFlag(state, 'flag-a', 40, 50); addTestFlag(state, 'flag-b', 50, 50); addTestFlag(state, 'flag-long', 59, 50); addTestFlag(state, 'flag-c', 45, 45); addTestFlag(state, 'flag-d', 45, 55);
    expect(isRoadPathValid(state, { id: 'too-short', startFlagId: 'flag-a', endFlagId: 'flag-b', cells: ['40-50'], active: true })).toBe(false);
    const twentyCells = Array.from({ length: 20 }, (_, i) => `${40 + i}-50`);
    ...
    const twentyOneCells = Array.from({ length: 21 }, (_, i) => `${40 + i}-50`);
