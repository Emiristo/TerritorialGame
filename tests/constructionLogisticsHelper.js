...
function ensureWarehouse(state, resourceId, amount) {
  ...
  if (!warehouse) {
    ...
    warehouse.active = true; warehouse.constructionComplete = true; state.buildings.push(warehouse);
  }
  warehouse.inventory ??= {};
  ...
  if (!flag) { flag = createFlag(`${warehouse.id}-flag`, warehouse.id, warehouse.ownerId, 5, 8); flag.constructionStorage = {}; state.flags.push(flag); }
...
function ensureConstructionFlag(state, building) {
  ...
  if (!flag) { building.flagId ??= `${building.id}-flag`; flag = createFlag(building.flagId, building.id, building.ownerId, 20 + state.flags.length, 8); flag.constructionStorage = {}; state.flags.push(flag); }
  building.flagId = flag.id; flag.constructionStorage ??= {}; return flag;
}
function ensureRoad(state, sourceFlag, destinationFlag, buildingId) {
  ...
  return addRoad(state, createRoad(`test-construction-road-${buildingId}-${state.roads.length}`, sourceFlag.id, destinationFlag.id, [`${sourceFlag.x}-${sourceFlag.y}`, `${destinationFlag.x}-${destinationFlag.y}`]));
}
export function deliverConstructionMaterialViaLogistics(state, building, resourceId, amount = 1) {
  ensurePlayer(state); const requested = Math.max(0, Math.floor(Number(amount) || 0)); let delivered = 0; const destinationFlag = ensureConstructionFlag(state, building);
  for (let unit = 0; unit < requested; unit += 1) {
    const { flag: warehouseFlag } = ensureWarehouse(state, resourceId, 1); const road = ensureRoad(state, warehouseFlag, destinationFlag, building.id); const before = state.transportRequests.length;
    ...
    if (!request) break;
    const carrier = createCarrier(`test-road-carrier-${building.id}-${resourceId}-${unit}-${state.carriers.length}`, state.player.id, road.id); addCarrier(state, carrier);
