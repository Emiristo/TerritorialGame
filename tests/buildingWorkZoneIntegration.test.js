...
import { advanceAllConstructions, assignConstructionWorker, deliverMaterialToConstructionFlag, startConstruction } from '../src/game/construction.js';
...
describe('building -> construction -> work zone -> worker', () => {
  it('creates a work zone on construction completion and assigns a compatible worker to it', () => {
    ...
    state.workers.push(createWorker('builder-1', 'player', 'builder'));
    state.workers.push(createWorker('worker-1', 'player', 'lumberjack'));
    expect(assignConstructionWorker(state, building.id, 'builder-1')).toBe(true);
    ...
    expect(state.workers.find((worker) => worker.id === 'worker-1')).toMatchObject({ buildingId: building.id, zoneId: zone.id, targetTileId: '46-45', state: 'working' });
