...
function place(state, id, typeId, tileId) {
  const [x, y] = tileId.split('-').map(Number);
  const type = Object.values(state.buildingTypes).find((item) => item.id === typeId) ?? null;
  const width = type?.width ?? 2;
  const height = type?.height ?? 2;
  prepareArea(state, x, y, width, height);
  return addBuilding(state, id, 'player', typeId, tileId);
}
...
describe('building logistics flags', () => {
  it('creates one flag for the headquarters on its south side', () => {
    ...
    expect(state.flags).toHaveLength(1); expect(flag).toMatchObject({ buildingId: headquarters.id, ownerId: 'player', x: 50, y: 52 });
  });
  it('creates a building flag automatically when the building is added', () => {
    const state = createGameState(); const building = place(state, 'building-2', 'stonecutter_hut', '30-30'); const flag = getFlagForBuilding(state, building.id);
  ...
  it('keeps automatic flag creation idempotent when synchronization runs', () => {
    const state = createGameState(); const building = place(state, 'building-2', 'stonecutter_hut', '30-30'); syncBuildingFlags(state);
  ...
  it('destroys only the building flag and its roads, keeping standalone flags and their roads', () => {
    const state = createGameState(); const building = place(state, 'building-2', 'stonecutter_hut', '31-31');
    ...
    addRoad(state, createRoad('building-road', hqFlag.id, buildingFlag.id, diagonalPath(50, 52, 32, 33))); addRoad(state, createRoad('standalone-road', standaloneA.id, standaloneB.id, ['20-20', '20-21', '20-22', '20-23', '20-24', '20-25'])); expect(areFlagsConnected(state, standaloneA.id, standaloneB.id)).toBe(true);
  ...
  it('destroys the building flag and its roads with the building', () => {
    const state = createGameState(); const building = place(state, 'building-2', 'stonecutter_hut', '31-31'); const buildingFlag = getFlagForBuilding(state, building.id); const headquartersFlag = getFlagForBuilding(state, state.buildings[0].id);
    addRoad(state, createRoad('road-1', headquartersFlag.id, buildingFlag.id, diagonalPath(50, 52, 32, 33))); expect(areFlagsConnected(state, headquartersFlag.id, buildingFlag.id)).toBe(true); destroyBuilding(state, building.id);
...
describe('logistics road network', () => {
  it('keeps disconnected flags disconnected', () => {
    const state = createGameState(); const headquartersFlag = getFlagForBuilding(state, state.buildings[0].id); const second = place(state, 'building-2', 'stonecutter_hut', '30-30'); const secondFlag = getFlagForBuilding(state, second.id); rebuildLogisticsNetwork(state);
  ...
  it('removes every road connected to a flag', () => {
    const state = createGameState(); const building = place(state, 'building-2', 'stonecutter_hut', '31-31'); const hqFlag = getFlagForBuilding(state, state.buildings[0].id); const buildingFlag = getFlagForBuilding(state, building.id);
    addRoad(state, createRoad('road-1', hqFlag.id, buildingFlag.id, diagonalPath(50, 52, 32, 33))); removeRoadsForFlag(state, buildingFlag.id); expect(state.roads).toHaveLength(0); expect(hqFlag.roadIds).toEqual([]);
  });
});
