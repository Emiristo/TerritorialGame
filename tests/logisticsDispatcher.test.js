...
function addMiddleFlag(state) {
  state.flags.push({ id: 'middle-flag', ownerId: state.player.id, buildingId: null, x: 46, y: 47, cargo: {} });
  return state.flags.find((flag) => flag.id === 'middle-flag');
}

describe('logistics dispatcher', () => {
  it('assigns a ready request to an existing road carrier', () => {
    const state = createGameState(); const warehouse = state.buildings.find((building) => building.typeId === BUILDING_TYPES.HEADQUARTERS.id); const destination = addDestinationBuilding(state);
    const warehouseFlag = state.flags.find((flag) => flag.buildingId === warehouse.id); const destinationFlag = state.flags.find((flag) => flag.buildingId === destination.id);
    const road = connectFlags(state, 'road-warehouse-destination', warehouseFlag.id, destinationFlag.id);
    const roadCarriersBefore = state.carriers.filter((carrier) => carrier.role === 'road').length;
    const request = createWarehouseTransportRequest(state, 'request-1', state.player.id, 'stone', 1, warehouse.id, destination.id); state.transportRequests.push(request);
    addCargoToFlag(state, warehouseFlag.id, 'stone', 1); request.state = 'ready'; request.routeFlagIds = [warehouseFlag.id, destinationFlag.id]; request.routeRoadIds = [road.id];
    expect(dispatchTransportRequests(state)).toBe(1); expect(state.carriers.filter((carrier) => carrier.role === 'road')).toHaveLength(roadCarriersBefore);
    expect(state.carriers.find((carrier) => carrier.role === 'road')?.cargo?.requestId).toBe('request-1'); expect(request.state).toBe('inTransit');
  });
  it('keeps one request alive while two road carriers execute two route stages', () => {
    const state = createGameState(); const warehouse = state.buildings.find((building) => building.typeId === BUILDING_TYPES.HEADQUARTERS.id); const destination = addDestinationBuilding(state);
    const warehouseFlag = state.flags.find((flag) => flag.buildingId === warehouse.id); const destinationFlag = state.flags.find((flag) => flag.buildingId === destination.id); const middleFlag = addMiddleFlag(state);
    ...
    expect(dispatchTransportRequests(state)).toBe(1); expect(advanceDispatchedCarriers(state)).toBe(1); expect(request.state).toBe('delivered'); expect(state.flags.find((flag) => flag.id === destinationFlag.id)?.cargo?.stone).toBe(1);
  });
});
