...
describe('buildings, workers and work zones', () => {
  ...
  it('places a resource building only on owned compatible terrain', () => { const state = createGameState(); const forest = state.tiles.find((tile) => tile.terrain === 'forest' && tile.ownerId === 'player'); const plains = state.tiles.find((tile) => tile.terrain === 'plains' && tile.ownerId === 'player'); expect(canBuildOnTile(state, BUILDING_TYPES.SAWMILL.id, forest.id)).toBe(true); expect(canBuildOnTile(state, BUILDING_TYPES.SAWMILL.id, plains.id)).toBe(false); expect(() => addBuilding(state, createBuilding('invalid', 'player', BUILDING_TYPES.SAWMILL.id, plains.id))).toThrow(); });
});
