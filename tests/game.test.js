...
import { WORKER_TYPES, createWorkZone, createWorker, assignWorkerToBuilding } from '../src/game/workers.js';
...
describe('building specifications', () => {
  const expected = {
    headquarters: [3, 3, {}], warehouse: [3, 3, { planks: 3, stone: 3 }], forester_hut: [2, 2, { planks: 2 }], stonecutter_hut: [2, 2, { planks: 2 }], sawmill: [2, 3, { planks: 2, stone: 2 }], well: [2, 2, { planks: 2 }], farm: [4, 4, { planks: 4, stone: 3 }], mill: [2, 2, { planks: 2, stone: 2 }], bakery: [2, 2, { planks: 2, stone: 2 }], coal_mine: [2, 2, { planks: 4 }], iron_mine: [2, 2, { planks: 4 }], gold_mine: [2, 2, { planks: 4 }], marble_mine: [2, 2, { planks: 4 }], foundry: [2, 2, { planks: 2, stone: 2 }], forge: [2, 2, { planks: 2, stone: 2 }], workshop: [3, 3, { planks: 2, stone: 2 }], mint: [2, 2, { planks: 2, stone: 2 }], outpost: [2, 2, { planks: 3 }], barracks: [3, 3, { planks: 3, stone: 1 }], watchtower: [2, 2, { planks: 5, stone: 5 }], fortress: [5, 5, { planks: 10, stone: 10 }],
  };
  ...
  it('keeps regular buildings on plains and mines on hills or mountains', () => { for (const type of Object.values(BUILDING_TYPES)) { if (type.role === 'military' || type.id === 'headquarters') continue; if (type.workerTypeId === 'miner') expect(type.terrainIds).toEqual(['hills', 'mountains']); else expect(type.terrainIds).toEqual(['plains']); } });
  it('keeps agreed work-zone and military values', () => { expect(BUILDING_TYPES.FORESTER_HUT.workRadius).toBe(8); expect(BUILDING_TYPES.STONECUTTER_HUT.workRadius).toBe(5); expect(BUILDING_TYPES.SAWMILL.workRadius).toBe(5); expect(BUILDING_TYPES.COAL_MINE.workRadius).toBe(5); expect(BUILDING_TYPES.IRON_MINE.workRadius).toBe(5); expect(BUILDING_TYPES.GOLD_MINE.workRadius).toBe(5); expect(BUILDING_TYPES.MARBLE_MINE.workRadius).toBe(5); expect(BUILDING_TYPES.FARM.workArea).toEqual({ width: 4, height: 4, usableCells: 14 }); expect(BUILDING_TYPES.OUTPOST).toMatchObject({ requiredSoldiers: 1, influenceMultiplier: 0.75 }); expect(BUILDING_TYPES.BARRACKS).toMatchObject({ requiredSoldiers: 3, influenceMultiplier: 1 }); expect(BUILDING_TYPES.WATCHTOWER).toMatchObject({ requiredSoldiers: 6, influenceMultiplier: 1.25 }); expect(BUILDING_TYPES.FORTRESS).toMatchObject({ requiredSoldiers: 9, influenceMultiplier: 1.5, blockChanceBonus: 0.05 }); });
});
...
describe('map and headquarters', () => {
  ...
  it('gives the headquarters influence radius 10', () => { const state = createGameState(); const source = state.territorySources.find((item) => item.id === 'headquarters-player'); expect(source.radius).toBe(10); expect(HEADQUARTERS_INFLUENCE_RADIUS).toBe(10); expect(state.tiles.find((tile) => tile.id === '60-50').ownerId).toBe('player'); expect(state.tiles.find((tile) => tile.id === '61-50').ownerId).not.toBe('player'); });
});
...
describe('building placement', () => {
  ...
  it('allows a mine on owned hills', () => { const state = createGameState(); const hill = state.tiles.find((tile) => tile.terrain === 'hills' && tile.x < 15 && tile.y > 30); expect(hill).toBeDefined(); for (const tile of getFootprintTiles(state, BUILDING_TYPES.IRON_MINE.id, hill.id)) tile.ownerId = 'player'; expect(canBuildOnTile(state, BUILDING_TYPES.IRON_MINE.id, hill.id)).toBe(true); });
});
