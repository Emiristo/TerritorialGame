...
describe('building catalog', () => {
  it('contains the agreed 22 building types', () => {
  ...
  it('returns defensive construction materials and resolves the type', () => {
  ...
  it('defines the lumberjack hut and sawmill roles', () => {
...
describe('building geometry and placement', () => {
  ...
  it('requires ownership and suitable terrain across the footprint', () => {
    const state = createGameState();
  ...
  it('blocks occupied footprints and preserves overlapping reservation zones', () => {
    const state = createGameState();
...
describe('physical construction mechanics', () => {
  ...
  it('delivers material to the flag without starting processing', () => {
    const building = constructionBuilding(BUILDING_TYPES.STONECUTTER_HUT.id);
  ...
  it('takes exactly one material from the flag before processing it', () => {
    const building = constructionBuilding(BUILDING_TYPES.STONECUTTER_HUT.id);
    const state = constructionState(building);
  ...
  it('accumulates delivered materials while the builder processes one', () => {
    const building = constructionBuilding(BUILDING_TYPES.WAREHOUSE.id);
    const state = constructionState(building);
    startConstruction(state, building);
  ...
  it('pauses and resumes only when the next material arrives', () => {
    const building = constructionBuilding(BUILDING_TYPES.WAREHOUSE.id);
    const state = constructionState(building);
    startConstruction(state, building);
    deliverConstructionMaterial(state, building.id, 'planks', 1);
  ...
  it('does not activate until every delivered unit is processed', () => {
    const building = constructionBuilding(BUILDING_TYPES.WAREHOUSE.id);
    const state = constructionState(building);
    startConstruction(state, building);
    ...
    advanceConstruction(state, building, 10);
    expect(building.constructionComplete).toBe(false);
    advanceConstruction(state, building, 20);
    expect(building.constructionComplete).toBe(false);
    advanceConstruction(state, building, 25);
  ...
  it('advances multiple buildings independently', () => {
    ...
    addBuilding(state, a);
    addBuilding(state, b);
    startConstruction(state, a);
    startConstruction(state, b);
  ...
  it('does not allow public completion while a material is being processed', () => {
    const building = constructionBuilding(BUILDING_TYPES.WAREHOUSE.id);
    const state = constructionState(building);
