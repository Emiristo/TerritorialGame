...
describe('physical construction mechanics', () => {
  ...
  it('does not deliver beyond the required material', () => { const { state, building } = constructionSetup('stonecutter_hut'); startConstruction(state, building); expect(deliverUnits(state, building, 'planks', 2)).toBe(2); expect(deliverConstructionMaterialViaLogistics(state, building, 'planks', 1)).toBe(0); });
  ...
  it('accumulates materials at the flag and processes them sequentially', () => { const { state, building } = constructionSetup('warehouse'); startConstruction(state, building); expect(deliverUnits(state, building, 'planks', 2)).toBe(2); expect(deliverConstructionMaterialViaLogistics(state, building, 'stone', 1)).toBe(1); advanceConstruction(state, building, 10); expect(building.currentConstructionMaterial).toBe('planks'); expect(state.flags.find((flag) => flag.buildingId === building.id).cargo.planks).toBe(1); advanceConstruction(state, building, 10); expect(building.currentConstructionMaterial).toBe('stone'); });
  ...
  it('activates only after every required unit is processed', () => { const { state, building } = constructionSetup('warehouse'); startConstruction(state, building); expect(deliverUnits(state, building, 'planks', 3)).toBe(3); expect(deliverUnits(state, building, 'stone', 3)).toBe(3); advanceConstruction(state, building, 75); expect(building.constructionComplete).toBe(true); expect(building.active).toBe(true); expect(building.constructionState).toBe(CONSTRUCTION_STATES.COMPLETED); });
  it('advances multiple constructions independently from warehouse stock', () => { const state = createGameState(); const a = place(state, 'a', 'stonecutter_hut', '30-30'); const b = place(state, 'b', 'stonecutter_hut', '40-40'); startConstruction(state, a); startConstruction(state, b); expect(deliverUnits(state, a, 'planks', 2)).toBe(2); expect(deliverUnits(state, b, 'planks', 2)).toBe(2); advanceAllConstructions(state, 20); expect(a.constructionComplete).toBe(true); expect(b.constructionComplete).toBe(true); });
  ...
  it('completes only fully processed construction and accepts an object or id', () => { const { state, building } = constructionSetup('stonecutter_hut'); startConstruction(state, building); expect(deliverUnits(state, building, 'planks', 2)).toBe(2); advanceConstruction(state, building, 20); expect(building.constructionComplete).toBe(true); expect(completeConstruction(state, building.id)).toBe(building); expect(building.active).toBe(true); });
});
