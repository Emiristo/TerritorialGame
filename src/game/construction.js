...
function getConstructionMaterials(building) { return { ...(building?.constructionMaterialsRequired ?? {}) }; }
function getMaterialDuration(resourceId) { return resourceId === 'planks' ? BUILD_TIME_PER_PLANK : resourceId === 'stone' ? BUILD_TIME_PER_STONE : 0; }
...
function allConstructionMaterialsProcessed(building) {
  return Object.entries(getConstructionMaterials(building)).every(([resource, amount]) => Number(building.constructionMaterialsDelivered?.[resource] ?? 0) >= Number(amount)) && !(building.constructionQueue ?? []).length && !building.currentConstructionMaterial;
}
...
export function advanceConstruction(state, building, elapsedSeconds) {
  ...
  while (remaining > 0 && building.currentConstructionMaterial) {
    ...
    if (building.currentConstructionMaterialRemainingTime === 0) {
      ...
      if (building.constructionQueue.length) startNextConstructionMaterial(building);
      else {
...
export function completeConstruction(state, buildingId, now = Date.now()) {
  ...
  if (!allConstructionMaterialsProcessed(building)) throw new Error('Construction materials are not fully processed');
