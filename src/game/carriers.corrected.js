...
export function createCarrier(id, ownerId, roadId = null) {
  return { id, ownerId, typeId: 'carrier', state: roadId ? CARRIER_STATES.WAITING : CARRIER_STATES.IDLE, roadId, cargo: null };
}

export function createTransportRequest(id, ownerId, resourceId, amount, sourceFlagId, destinationFlagId) {
  return { id, ownerId, resourceId, amount: Math.max(0, Math.floor(Number(amount) || 0)), delivered: 0, sourceFlagId, destinationFlagId, routeFlagIds: [], state: 'waiting' };
}
...
export function removeCarrierFromRoad(state, carrierId) {
  ...
  carrier.roadId = null; carrier.state = CARRIER_STATES.IDLE; return true;
}
...
export function prepareTransportRequest(state, request) {
  ...
  if (request.ownerId !== state.player?.id) return false;
  ...
  request.routeFlagIds = route.flagIds;
  request.routeRoadIds = route.roadIds;
  request.state = 'ready';
  return true;
}
