...
import { getFlagCargoCount, getFlagFreeCargoCapacity } from './flagCargo.js';
...
export function getFlagCargoCapacity() { return 20; }
export function getFlagCargoCountForFlag(state, flagId) { return getFlagCargoCount(getFlag(state, flagId)); }
export function getFlagFreeCargoCapacityForFlag(state, flagId) { return getFlagFreeCargoCapacity(getFlag(state, flagId)); }
export function addCargoToFlag(state, flagId, resourceId, amount = 1) { const flag = getFlag(state, flagId), units = Math.max(0, Math.floor(Number(amount) || 0)); if (!flag || !resourceId || units <= 0 || getFlagFreeCargoCapacity(flag) < units) return 0; const cargo = ensureFlagCargo(flag); cargo[resourceId] = Number(cargo[resourceId] ?? 0) + units; return units; }
...
export function deliverCarrierToFlag(state, carrierId) {
  ...
  if (constructionBuilding?.constructionComplete === false && destination.id === request.destinationFlagId) return deliverCarrierToConstructionFlag(state, carrierId);
  ...
  if (destination.id === request.destinationFlagId) { request.currentSegmentIndex = request.routeRoadIds.length; request.state = 'at_destination'; }
  else { request.currentSegmentIndex = Number(cargo.segmentIndex) + 1; request.state = 'ready'; }
  carrier.cargo = null;
...
export function deliverCarrierToConstructionFlag(state, carrierId) {
  ...
  if (delivered !== cargo.amount) { removeCargoFromFlag(state, destination.id, cargo.resourceId, cargo.amount); return false; }
  request.inTransit = Math.max(0, Number(request.inTransit ?? 0) - cargo.amount);
