...
export function createTransportRequest(id, ownerId, resourceId, amount, sourceFlagId, destinationFlagId) {
  return { id, ownerId, resourceId, amount: Math.max(0, Math.floor(Number(amount) || 0)), delivered: 0, inTransit: 0, sourceFlagId, destinationFlagId, routeFlagIds: [], routeRoadIds: [], state: 'waiting' };
}
export function createBuildingTransportRequest(state, id, ownerId, resourceId, amount, sourceBuildingId, destinationBuildingId) {
  ...
  const request = createTransportRequest(id, ownerId, resourceId, amount, sourceFlag.id, destinationFlag.id);
  request.sourceBuildingId = sourceBuildingId; request.destinationBuildingId = destinationBuildingId;
  return request;
}
export function createWarehouseTransportRequest(state, id, ownerId, resourceId, amount, warehouseBuildingId, destinationBuildingId) {
  const request = createBuildingTransportRequest(state, id, ownerId, resourceId, amount, warehouseBuildingId, destinationBuildingId);
  if (!request) return null;
  request.sourceWarehouseId = warehouseBuildingId;
  return request;
}
...
export function addCarrier(state, carrier) {
  ...
  if (carrier.role === CARRIER_ROLES.ROAD && carrier.roadId && !getRoad(state, carrier.roadId)) throw new Error('Carrier road does not exist');
  if (carrier.role === CARRIER_ROLES.ROAD && carrier.roadId && carrierCountOnRoad(state, carrier.roadId) >= getRoadCarrierCapacity(state, carrier.roadId)) throw new Error('Road has reached its carrier capacity');
...
export function assignCarrierToRoad(state, carrierId, roadId) {
  ...
  if (!carrier || carrier.role !== CARRIER_ROLES.ROAD || !road || carrier.cargo) return false;
...
export function getWarehouseCarrier(state, warehouseBuildingId) {
  ...
  const existing = (state.carriers ?? []).find((carrier) => carrier.role === CARRIER_ROLES.WAREHOUSE && carrier.buildingId === warehouseBuildingId);
  if (existing) return existing;
  const carrier = createWarehouseCarrier(`warehouse-carrier-${warehouseBuildingId}`, warehouse.ownerId, warehouseBuildingId);
  addCarrier(state, carrier);
  warehouse.warehouseCarrierId = carrier.id;
  return carrier;
}
...
export function deliverFlagCargoToBuilding(state, flagId, resourceId, amount = 1) { const flag = getFlag(state, flagId); if (!flag?.buildingId) return 0; const units = removeCargoFromFlag(state, flagId, resourceId, amount); if (units <= 0) return 0; return addInventoryToBuilding(state, flag.buildingId, resourceId, units); }
...
export function stageWarehouseCargoForRequest(state, requestId) {
  ...
  request.state = 'ready'; request.sourceStaged = true; carrier.cargo = { requestId, resourceId: request.resourceId, amount: 1, fromBuildingId: request.sourceWarehouseId, toFlagId: request.sourceFlagId }; carrier.state = CARRIER_STATES.CARRYING;
  return true;
}
export function completeWarehousePickup(state, requestId) {
  const request = (state.transportRequests ?? []).find((item) => item.id === requestId) ?? null;
  const carrier = (state.carriers ?? []).find((item) => item.role === CARRIER_ROLES.WAREHOUSE && item.cargo?.requestId === requestId) ?? null;
  if (!request || !carrier) return false;
  carrier.cargo = null; carrier.state = CARRIER_STATES.IDLE; return true;
}
...
export function deliverCarrierToFlag(state, carrierId) {
  const carrier = getCarrier(state, carrierId); if (!carrier?.cargo || carrier.role !== CARRIER_ROLES.ROAD) return false;
  const cargo = carrier.cargo, request = (state.transportRequests ?? []).find((item) => item.id === cargo.requestId) ?? null, destination = getFlag(state, cargo.toFlagId); if (!request || !destination) return false;
  addCargoToFlag(state, destination.id, cargo.resourceId, cargo.amount); request.inTransit = Math.max(0, Number(request.inTransit ?? 0) - cargo.amount); recordRoadCargo(state, cargo.roadId, cargo.amount);
  if (destination.id === request.destinationFlagId) request.state = 'at_destination';
