...
export function renderMap(container, state, selectedBuildingTypeId = null, previewTileId = null) {
  ...
  if (!worldMap) return;
  container.style.setProperty('--map-columns', String(worldMap.width));
  ...
  for (const tile of worldMap.tiles) {
    ...
    const roads = getRoadAtTile(state, tile.id);
    ...
    button.type = 'button';
    button.className = ['tile', `terrain-${tile.terrain}`, ownerClass, tile.id === state.selectedTileId ? 'is-selected' : '', source ? 'is-territory-source' : '', building ? 'is-building' : '', roads.length ? 'is-road' : '', previewClass].filter(Boolean).join(' ');
    button.dataset.tileId = tile.id;
    button.setAttribute('role', 'gridcell');
    button.setAttribute('aria-label', `${terrain?.name ?? tile.terrain}, клетка ${tile.id}, владелец: ${tile.ownerId ?? 'нет'}`);
    button.title = building ? `${BUILDING_SYMBOLS[building.typeId] ?? '⌂'} ${building.typeId} · ${tile.x}, ${tile.y}` : `${terrain?.name ?? tile.terrain} · ${tile.x}, ${tile.y}`;
    button.textContent = building ? (BUILDING_SYMBOLS[building.typeId] ?? '⌂') : (roads.length ? '━' : (source ? `◆${TERRAIN_SYMBOLS[tile.terrain] ?? ''}` : (TERRAIN_SYMBOLS[tile.terrain] ?? '')));
    container.append(button);
  }
  for (const flag of state.flags ?? []) {
    const node = document.createElement('button');
    node.type = 'button';
    node.className = `map-flag-node${flag.id === state.selectedFlagId ? ' is-selected' : ''}`;
    node.textContent = '⚑';
    node.title = flag.buildingId ? `Флаг здания: ${flag.buildingId}` : 'Самостоятельный флаг';
    node.setAttribute('aria-label', node.title);
    node.dataset.flagId = flag.id;
    node.style.position = 'absolute';
    node.style.left = `${(flag.x / worldMap.width) * 100}%`;
    node.style.top = `${(flag.y / worldMap.height) * 100}%`;
