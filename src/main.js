...
function getBuildableType() { return Object.values(BUILDING_TYPES).find((type) => type.id !== 'headquarters' && canAfford(type.cost)); }

let selectedBuildingTypeId = null;

elements.map.addEventListener('click', (event) => {
  const tile = event.target.closest('[data-tile-id]'); if (!tile) return;
...
elements.tilePanel.addEventListener('click', (event) => {
  ...
  if (action.dataset.action === 'build') {
    const tile = getSelectedTile(state); const typeId = selectedBuildingTypeId;
    if (!tile || !typeId) return;
    ...
    pay(type.cost); addBuilding(state, createBuilding(`building-${state.buildings.length + 1}`, state.player.id, typeId, tile.id));
    elements.status.textContent = `Построено: ${type.name}.`; selectedBuildingTypeId = null; render();
  }
});
